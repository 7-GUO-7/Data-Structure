...
public:
    ...
    void search(node *&tree_node,int x)
    {
        if (find(tree_node,x))
            cout<<'Y'<<endl;
        else
    ...
    void find_ith(search_tree *search_tree,int i)
    {
        struct StNode
        {
            node *node1;
        ...
        if (search_tree->current_node<i) {cout<<'N'<<endl;return;}
        int tmp[5100]={0};
        int count=0;
